...
bool math::world_to_screen(const vec3_t& origin, vec2_t& screen) {
	...
	if (!view_matrix)
		view_matrix = *reinterpret_cast<std::uintptr_t*>(reinterpret_cast<std::uintptr_t>(gUtils.PatternScan("client.dll", "0F 10 05 ? ? ? ? 8D 85 ? ? ? ? B9")) + 3) + 176;
	...
	int x, y;
	gInterfaces.engine->getScreenSize(x, y);
